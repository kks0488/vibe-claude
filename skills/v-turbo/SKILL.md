...
1. **Independent tasks → parallel launch** (fan-out pattern)
2. **Background long tasks**: npm install, builds, tests → `run_in_background: true`
3. **Phase 1**: ALL recon agents simultaneously
4. **Phase 3**: Multiple v-workers + v-designer in parallel
5. **Phase 4**: v-critic + v-analyst + tests simultaneously
6. **Never wait when you can work** — zero idle time
7. **Show parallel execution timestamps as evidence**

## Agent Dispatch

| Task | Agent | Background? |
|------|-------|-------------|
| Deep analysis | v-analyst | No |
| Quick search | v-finder | No |
| Code writing | v-worker | No |
| UI work | v-designer | No |
| Documentation | v-writer | Yes |
| Build/test/install | Bash | Yes |
